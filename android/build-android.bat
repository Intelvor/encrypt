@echo off
rem Android APK build script (aapt2 + d8 + zipalign + apksigner).
...
if not exist "%KEYSTORE%" (
    ...
    echo   keytool -genkeypair -storetype PKCS12 -alias encrypt -keyalg RSA -keysize 2048
    echo         -validity 10000 -storepass %KEYSTORE_PASS% -keypass %KEYSTORE_PASS%
...
echo === Step 6: sign ===
call "%APKSIGNER%" sign --ks "%KEYSTORE%" --ks-key-alias encrypt --ks-pass pass:%KEYSTORE_PASS% --key-pass pass:%KEYSTORE_PASS% --out "%OUT%\encrypt.apk" "%OUT%\aligned.apk"
if errorlevel 1 goto fail
